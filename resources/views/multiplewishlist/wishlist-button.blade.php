...
<toggler v-slot="{ isOpen, toggle, close }" v-cloak>
    <div class="absolute right-0 top-0">
        ...
        <div v-show="isOpen" v-if="$root.user" v-cloak class="absolute right-0 border rounded-md bg-gray-100 p-5 w-64">
            <lazy>
                <wishlist v-slot="{ wishlists, toggleItem, contains, addWishlist }">
                    <div class="flex flex-col gap-2">
                        <div v-for="wishlist in wishlists" :key="wishlist.id" class="flex gap-2 w-full items-center justify-end">
                            <a class="overflow-ellipsis overflow-hidden whitespace-nowrap" :href="'/account/wishlists/' + wishlist.id">
                                @{{ wishlist.title }}
                            </a>
                            <div class="border border-primary rounded-md w-fit inline-block self-start transition" :class="contains(wishlist, {{ $productId }}) ? 'text-white bg-primary' : 'text-primary'">
