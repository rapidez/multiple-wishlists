                ...
                <div class="flex flex-col" v-if="wishlists">
                    ...
                    >
                        ...
                        <div class="flex m-2 gap-2 h-10 self-center justify-end items-center w-1/5">
                            <a :href="'{{ route('wishlist.shared', '') }}/' + wishlist.sharing_token" v-if="wishlist.shared" class="text-primary hover:underline">@lang('(shared)')</a>
                            <x-rapidez::button v-if="wishlist" ::href="'{{ route('wishlist.edit', '') }}/' + wishlist.id">@lang('Edit')</x-rapidez::button>
                            <x-rapidez::button variant="outline" v-if="wishlist" @click="removeWishlist(wishlist)">@lang('Delete')</x-rapidez::button>
                        </div>
                    </div>
                </div>
                <x-rapidez::button variant="outline" class="self-start mt-2" @click="addWishlist('{{ __('New wishlist') }}')">@lang('Create new wishlist')</x-rapidez::button>
            </div>
        </wishlist>
    </div>
@endsection
